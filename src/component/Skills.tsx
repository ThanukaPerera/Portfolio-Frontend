...
const categorizeSkills = (skills: string[] = []) => {
  ...
  const categorized = {
    "Web & Mobile": skills.filter(
      (skill) => categoryMap[skill] === "Web & Mobile"
    ),
...
export default function Skills({ data }: SkillsProps) {
  if (!data || !data.skills || data.skills.length === 0)
    return <div className="text-white">No skills available</div>;

  const categorized = categorizeSkills(data.skills);

  return (
    <div className="relative min-h-screen overflow-hidden max-w-full text-white">
      <div className="relative z-10 flex flex-col items-center justify-center mx-auto sm:max-w-7xl max-w-full py-16 px-8 max-sm:px-6">
        {/* Section Title */}
        <motion.div
          initial={{ opacity: 0, y: -30 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        <motion.div
          ...
          initial="hidden"
          animate="visible"
          className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-6 maw-full sm:max-w-5xl  mx-auto"
        >
          {["Web & Mobile", "Languages", "Databases", "Tools"].map(
            (category) => {
              const skillsArray =
              ...
              return (
                <motion.div
                  key={category}
                  variants={skillVariants}
                  className="bg-gray-900/60 backdrop-blur-sm border w-full border-gray-700/50 rounded-xl p-6 hover:border-gray-600/50 transition-all duration-300 shadow-lg"
                >
                  {/* Category Title */}
                  <div className="text-center mb-6">
                    <h3
                      className={`text-xl font-bold text-white mb-2 ${montserrat.className}`}
                    >
                      {category}
                    </h3>
                  </div>
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center space-x-3">
                            ...
                            {/* Skill Name */}
                            <h4
                              className={`text-sm font-medium text-gray-200 group-hover:text-white transition-colors duration-300 truncate ${montserrat.className}`}
                            >
                              {skill}
                            </h4>
                          </div>
